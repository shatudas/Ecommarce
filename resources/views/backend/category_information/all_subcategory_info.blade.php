...
<head>
	...
	<title>View all Subcategory</title>
				...
				<div class="card-body" style="overflow-x:auto; width: 100%; margin-top: -25px;">
					<table id="bs4-table" class="table table-bordered table-striped mt-2 bg-white" cellspacing="0" style="font-size: 13px;">
						...
						<tbody>
							...
							@if(isset($all_subcat))
							@foreach($all_subcat as $sucate_data)
							<tr>
								<td>{{ $sucate_data->id }}</td>
								<td>{{ $sucate_data->sl }}</td>
								<td>{{ $sucate_data->item_name }}</td>
								<td>{{ $sucate_data->category_name }}</td>
								<td>{{ $sucate_data->subcategory_name }}</td>
								<td align="center">
									@if($sucate_data->status==1)
									  Active
									 @else
									  Inactive
									@endif
