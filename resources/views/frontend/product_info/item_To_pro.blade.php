	...
	<div class="container-fluid">
		...
		<div>
			<ul class="uk-breadcrumb" style="display: flex; flex-wrap: wrap;list-style: none;">
				...
				<i style="margin-top:3px;">|</i> &nbsp;&nbsp;&nbsp;
				<li>
					<span style="color:gray">{{ $itemName->item_name }}</span>
				</li>
			</ul>
		</div>
			...
			<div class="col-lg-2 col-md-12 col-sm-12 col-12 p-0 "  style="border-radius:0px;">
				...
				<div class="list-group bg-dark " style="border-radius:0px;">
					<a href="#" class="list-group-item  active  border-0 text-uppercase" style="border-radius:0px; background-color:#2E4053;">
						<center>Categorys</center>
					</a>

					<div class="sidebarcat">
						@foreach($catName as $catname)
						<a href="{{ route('catProduct',$catname->id) }}" class="list-group-item list-group-item-action">{{ $catname->category_name }}</a>
						@endforeach
					</div>
				</div>
			...
			<div class="col-lg-10 col-md-12 col-sm-12 col-12">
				<div class="col-sm-12 col-12 pa">
					<div class="row"  id="showdata">
						...
						@if(isset($itempro))
						@foreach($itempro as $prodata)

						<div class="col-lg-3 cl-md-4 col-sm-6 col-6 mt-4">

							@if(isset($prodata->Discount_price))
							@php
							$dis=$prodata->Discount_price / $prodata->sale_price*100;
							@endphp
							...
							<div class="homeproducts border">
								<center>
									<a href="{{ route('product_view',$prodata->id) }}">
										<img src="{{ asset($prodata->product_image) }}" class="img-fluid" style=""></a>
								</center>
								<div>
									<a href="{{ route('product_view',$prodata->id) }}">
										<center>
										 @php
						      $productName = substr($prodata->product_name,0,20);
						     @endphp
										  {{$productName}}..<br>

										  @php
										  $old_price=$prodata->sale_price;
										  $dis_price=$prodata->Discount_price;
										  $present_price=$old_price-$dis_price;
										  @endphp

										 @if(isset($prodata->Discount_price))
						      <del>{{ $prodata->sale_price }}</del><br/>
						      @else	<br/>
						     @endif
