...
//-----user dashboard-----------//
 public function userdashboardmethod(){
	...
	return view('frontend.user_dashboard.user_dashboard',compact('adddata','Pending','processing','Shiping','complated'));
}

public function all_ordermethod(){
$adddata = DB::table('invoece')->where('use_id',Auth()->user()->id)->get();
	return view('frontend.user_dashboard.all_order',compact('adddata'));
}

public function updateinformationmethod(){
	return view('frontend.user_dashboard.updateinformation');
}
...
public function invoicemethod($id){
 ...
 ->get();
 return view('frontend.user_dashboard.invoice_view',compact('adddata','data'));
}
